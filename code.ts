...
for (var i = 0; i < totalIds; i++) {
  ...
  let unstyledFont = textStyleIds.unstyledFont[i];
...
function styleText(nodes) {
  for (const node of nodes) {
    if (node.type === 'TEXT') {
      console.log(node.fontSize, node.fontName);
      if (textStyleMapUnstyledToLexicon.has(node.fontName)) {
        console.log("yeet");
