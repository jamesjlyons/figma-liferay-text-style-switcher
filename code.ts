...
let textStyleMapUnstyledToLexicon = new Map();
...
for (var i = 0; i < totalIds; i++) {
  ...
  let unstyledFont = textStyleIds.unstyledFonts[i];

  // Map lexicon style ids to corresponding ac style ids
  if (lexiconTextId.key === acTextId.key) {
    textStyleMapLexiconToAc.set(
      lexiconTextId.textStyleId,
      acTextId.textStyleId
    );
  } else {
    console.error(
      "Text style key mismatch:" + lexiconTextId.key + ", " + acTextId.key
    );
  }

  // Map ac style ids to corresponding lexicon style ids
  if (acTextId.key === lexiconTextId.key) {
    textStyleMapAcToLexicon.set(
      acTextId.textStyleId,
      lexiconTextId.textStyleId
    );
  } else {
    console.error(
      "Text style key mismatch:" + acTextId.key + ", " + lexiconTextId.key
    );
  }

  // WIP --- Map unstyled to corresponding lexicon style ids
  if (unstyledFont.key === lexiconTextId.key) {
    textStyleMapUnstyledToLexicon
      .set(unstyledFont.fontSize,
        lexiconTextId.textStyleId
        ).set(lexiconTextId.textStyleId,
          unstyledFont.fontName
          );
  } else {
    console.error(
      "Text style key mismatch:" + unstyledFont.key + ", " + lexiconTextId.key
    );
  }
}
...
function changedFontsActions() {
  changed = true;
  changedLayers++;
}

function unchangedFontsActions() {
  error = true;
  unchangedLayers++;
}
...
// WIP --- Map unstyled to corresponding lexicon style ids
function styleText(nodes) {
  for (const node of nodes) {
    if ("textStyleId" in node) {
      console.log(node.fontSize, node.fontName, node.textStyleId);
      if (textStyleMapUnstyledToLexicon.has(node.textFontSize), textStyleMapUnstyledToLexicon.has(node.textFontName)) {
        console.log("yes");
        node.textStyleId = textStyleMapUnstyledToLexicon.get(node.textStyleId);
        // changedFontsActions();
      } else {
        console.warn("nah");
        // unchangedFontsActions();
