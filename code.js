...
for (var i = 0; i < totalIds; i++) {
    ...
    if (unstyledFont.key === lexiconTextId.key) {
        textStyleMapUnstyledToLexicon
            .set(unstyledFont.fontSize, lexiconTextId.textStyleId).set(lexiconTextId.textStyleId, unstyledFont.fontName);
    }
    else {
        console.error("Text style key mismatch:" + unstyledFont.key + ", " + lexiconTextId.key);
    }
}
function notifyAc() {
    if (changed && error) {
        figma.notify(changedLayers +
            " layers changed, " +
            unchangedLayers +
            " missing corresponding Lexicon text style", { timeout: notificationTime });
        changed = false;
        error = false;
    }
    else if (changed) {
...
function notifyLexicon() {
    if (changed && error) {
        figma.notify(changedLayers +
            " layers changed, " +
            unchangedLayers +
            " missing corresponding AC text style", { timeout: notificationTime });
        changed = false;
        error = false;
    }
    else if (changed) {
